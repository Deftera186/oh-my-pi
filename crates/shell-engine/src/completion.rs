...
impl Spec {
	...
	pub async fn get_completions(
		...
		if candidates.is_empty() && options.bash_default {
			let Answer::Candidates(mut fallback, fallback_options) =
				get_completions_using_basic_lookup(shell, context).await
			else {
				unreachable!("basic completion never restarts completion");
			};
			candidates.append(&mut fallback);
			processing_options.treat_as_filenames |= fallback_options.treat_as_filenames;
			processing_options.no_autoquote_filenames |= fallback_options.no_autoquote_filenames;
			processing_options.no_trailing_space_at_end_of_line |=
				fallback_options.no_trailing_space_at_end_of_line;
		}
		...
		for action in &self.actions {
			match action {
				...
				CompleteAction::Service => {
					candidates.extend(get_service_completions(token));
				},
	...
	async fn call_completion_function(
		...
		// NOTE: Don't pollute the persistent environment with these?
		let vars_and_values: Vec<(&str, ShellValueLiteral)> = vec![
			("COMP_LINE", context.input_line.into()),
			("COMP_POINT", context.cursor_index.to_string().into()),
			("COMP_KEY", context.trigger.comp_key().to_string().into()),
	...
	pub async fn get_completions(
		...
		for (i, token) in tokens.iter().enumerate() {
			...
			if cursor < token.start {
				// NOTE: Should insert an empty token here; the position looks to
...
fn get_service_completions(prefix: &str) -> Vec<String> {
	get_service_completions_in(prefix, [
		"/etc/systemd/system",
		"/run/systemd/system",
...
async fn get_completions_using_basic_lookup(
	...
	// completions too.
	// NOTE: Do a better job than just checking if index == 0.
...
fn completion_filter_pattern_matches(
	...
	//
	// NOTE: Replace unescaped '&' with the word being completed.
	//
...
mod tests {
	...
	#[test]
	fn service_completion_reads_systemd_and_launchd_names() {
		let directory = tempfile::tempdir().unwrap();
		std::fs::write(directory.path().join("alpha.service"), "").unwrap();
		std::fs::write(directory.path().join("alpine.plist"), "").unwrap();
		std::fs::write(directory.path().join("beta.service"), "").unwrap();
		assert_eq!(get_service_completions_in("al", [directory.path()]), ["alpha", "alpine"]);
	}
}
