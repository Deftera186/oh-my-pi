//! Data types shared across the git and jj backends.
//!
//! Shapes consumed by the coding agent's commit pipeline, status line, review
//! flow, and worktree isolation machinery.
...
///
/// The output contract is git patch format — consumers parse it with the same
/// parsers they apply to `git diff` output, so header lines, `index`/mode
/// lines, `GIT binary patch` blocks, and rename headers must match the CLI
/// dialect.
