...
about.
Theme enumeration, raw palette inspection, and theme switching are deliberately not
exposed. Appearance is a user-owned preference: extensions style with semantic `ui.Token`
values and may read only the coarse `Appearance` from `await ui.presentation()` or
`RenderCtx.appearance` when content itself must differ between dark and light themes.
...
Global tool-output expansion is deliberately not exposed as either a getter or a setter.
`RenderCtx.collapsed` is the host-owned, read-only fold input for this entry; a renderer
emits its compact summary or expanded detail accordingly and cannot change another entry
or the user's fold policy.
...
| `ctx.ui.setTheme` / `getAllThemes` / `getTheme` | **Dropped** | Appearance is user-owned. Use semantic `ui.Token` values, with read-only `Appearance` from `ui.presentation()` or `RenderCtx.appearance` only for content selection. |
