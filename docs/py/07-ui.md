...
one through eight frames, each at most eight terminal display columns according to
`xutf`; invalid input is refused as `InvalidWorkingIndicator`. Core owns the clock and
selects frames at `interval_ms` (or its default cadence), so Python is never entered per
frame. The override is cleared automatically when the active turn settles, exactly like
the working-message override.
