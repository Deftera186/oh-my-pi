...
| LSP / DAP adapter | The document authority starts configured executables with piped stdio (`crates/docserver/src/lsp_process.rs`, `crates/docserver/src/dap_protocol.rs`). | Document-owned session tasks frame requests, dispatch responses/events, and terminate child processes with the authority. |
...
| envd ↔ document authority | Owner-local UDS or Windows pipe in project mode; Tokio duplex in local embedded mode. Bounded length-delimited protobuf (`crates/docserver/src/connection.rs`, `crates/envd/src/docs.rs`). | `omp.document.v1.ClientFrame` / `ServerFrame`: document leases, revisioned reads/edits/transactions, filesystem operations, watches, LSP and DAP operations/events. |
...
| document authority ↔ LSP/DAP | Piped stdio with `Content-Length` JSON-RPC framing (`crates/docserver/src/lsp_process.rs`, `crates/docserver/src/dap_protocol.rs`). | Standard LSP or DAP JSON messages, not OMP protobuf. |
...
| Document authority connection | `crates/docserver/src/connection.rs` |
